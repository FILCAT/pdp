...
contract PDPServiceProofSetCreateDeleteTest is Test {
    ...
    function testCreateProofSet() public {
        ...
        assertEq(
            ...
            0,
            "Proof set leaf count should be 0"
        );

        (address owner, address proposedOwner) = pdpService.getProofSetOwner(
            setId
        );
        assertEq(
            owner,
            address(this),
            "Proof set owner should be the constructor sender"
        );
        assertEq(
            proposedOwner,
            address(0),
            "Proof set proposed owner should be initialized to zero address"
        );

        assertEq(
            pdpService.getNextChallengeEpoch(setId),
            0,
            "Proof set challenge epoch should be zero"
        );
        assertEq(
            pdpService.rootLive(setId, 0),
            false,
            "Proof set root should not be live"
        );
        assertEq(
            pdpService.getRootCid(setId, 0).data,
            zeroRoot.data,
            "Uninitialized root should be empty"
        );
        assertEq(
            pdpService.getRootLeafCount(setId, 0),
            0,
            "Uninitialized root should have zero leaves"
        );
        assertEq(
            pdpService.getNextChallengeEpoch(setId),
            0,
    ...
    function testGetProofSetID() public {
        ...
        assertEq(
            2,
            pdpService.getNextProofSetId(),
            "Next proof set ID should be 2"
        );
        tearDown();
    }
}
...
contract PDPServiceOwnershipTest is Test {
    ...
    function testOwnershipTransfer() public {
        ...
        (address ownerStart, address proposedOwnerStart) = pdpService
            .getProofSetOwner(setId);
        assertEq(
            ownerStart,
            owner,
            "Proof set owner should be the constructor sender"
        ...
        assertEq(
            proposedOwnerStart,
            nextOwner,
            "Proof set proposed owner should make the one proposed"
        );
        vm.prank(nextOwner);
        pdpService.claimProofSetOwnership(setId);
        (address ownerEnd, address proposedOwnerEnd) = pdpService
            .getProofSetOwner(setId);
        assertEq(
            ownerEnd,
            nextOwner,
            "Proof set owner should be the next owner"
        );
        assertEq(
            proposedOwnerEnd,
            address(0),
            "Proof set proposed owner should be zero address"
        );
    }
...
contract PDPServiceProofSetMutateTest is Test {
    ...
    function testAddMultipleRoots() public {
        ...
        roots[1] = PDPService.RootData(
            Cids.Cid(abi.encodePacked("test2")),
            128
        );
        ...
        uint256 expectedLeafCount = roots[0].rawSize /
            32 +
            roots[1].rawSize /
            32;
        ...
        assertEq(
            pdpService.getRootCid(setId, firstId).data,
            roots[0].root.data
        );
        assertEq(
            pdpService.getRootCid(setId, firstId + 1).data,
            roots[1].root.data
        );

        assertEq(
            pdpService.getRootLeafCount(setId, firstId),
            roots[0].rawSize / 32
        );
        assertEq(
            pdpService.getRootLeafCount(setId, firstId + 1),
            roots[1].rawSize / 32
        );
    ...
    function expectIndexedError(
        uint256 index,
        string memory expectedMessage
    ) internal {
        vm.expectRevert(
            abi.encodeWithSelector(
                PDPService.IndexedError.selector,
                index,
                expectedMessage
            )
        );
    ...
    function testAddBadRoot() public {
        ...
        roots[0] = PDPService.RootData(
            Cids.Cid(abi.encodePacked("test")),
            pdpService.MAX_ROOT_SIZE() + 32
        );
...
contract PDPServiceProofTest is Test {
    ...
    function testProveSingleRoot() public {
        uint leafCount = 10;
        (uint256 setId, bytes32[][] memory tree) = makeProofSetWithOneRoot(
            leafCount
        );
        ...
        PDPService.Proof[] memory proofs = buildProofsForSingleton(
            setId,
            challengeCount,
            tree,
            leafCount
        ...
        recordAssert.expectRecord(
            PDPListener.OperationType.PROVE_POSSESSION,
            setId
        );
    ...
    function testBadRootsRejected() public {
        ...
        uint256 challengeEpoch = pdpService.getNextChallengeEpoch(setId);
        vm.roll(challengeEpoch);
        PDPService.Proof[] memory proofsOneRoot = buildProofsForSingleton(
            setId,
            1,
            trees[0],
            leafCounts[0]
        ...
        PDPService.Proof[] memory proofsTwoRoots = buildProofs(
            setId,
            1,
            trees,
            leafCounts
    ...
    function testProveManyRoots() public {
        ...
        (uint256 setId, bytes32[][][] memory trees) = makeProofSetWithRoots(
            leafCounts
        );
        ...
        PDPService.Proof[] memory proofs = buildProofs(
            setId,
            challengeCount,
            trees,
            leafCounts
    ...
    function makeProofSetWithOneRoot(
        uint leafCount
    ) internal returns (uint256, bytes32[][] memory) {
    ...
    function addOneRoot(
        uint256 setId,
        uint leafCount
    ) internal returns (bytes32[][] memory, uint256) {
    ...
    function makeRoot(
        bytes32[][] memory tree,
        uint leafCount
    ) internal pure returns (PDPService.RootData memory) {
        return
            PDPService.RootData(
                Cids.cidFromDigest(bytes(cidPrefix), tree[0][0]),
                leafCount * 32
            );
    }
    ...
    function buildProofs(
        ...
        uint[] memory leafCounts
    ) internal view returns (PDPService.Proof[] memory) {
        uint256 challengeEpoch = pdpService.getNextChallengeEpoch(setId);
        ...
        PDPService.Proof[] memory proofs = new PDPService.Proof[](
            challengeCount
        );
        for (
            uint challengeIdx = 0;
            challengeIdx < challengeCount;
            challengeIdx++
        ) {
            ...
            uint256 challengeOffset = uint256(keccak256(payload)) %
                totalLeafCount;
            uint treeIdx = 0;
            ...
            proofs[challengeIdx] = PDPService.Proof(
                tree[tree.length - 1][treeOffset],
                path
            );
        }
        return proofs;
    ...
    function buildProofsForSingleton(
        uint256 setId,
        uint challengeCount,
        bytes32[][] memory tree,
        uint leafCount
    ...
    function testHeightFromIndex() public view {
        ...
        for (uint256 i = 0; i < 105; i++) {
            assertEq(
                ...
                pdpService.testHeightFromIndex(i) + 1,
...
contract SumTreeAddTest is Test {
    ...
    function testMultiAdd() public {
        ...
        PDPService.RootData[] memory rootDataArray = new PDPService.RootData[](
            8
        );

        for (uint256 i = 0; i < counts.length; i++) {
            Cids.Cid memory testCid = Cids.Cid(abi.encodePacked("test", i));
            rootDataArray[i] = PDPService.RootData(
                testCid,
                counts[i] * pdpService.LEAF_SIZE()
            );
        }
        pdpService.addRoots(testSetId, rootDataArray);
        assertEq(
            pdpService.getProofSetLeafCount(testSetId),
            87,
            "Incorrect final proof set leaf count"
        );
        assertEq(
            pdpService.getNextRootId(testSetId),
            8,
            "Incorrect next root ID"
        );
        assertEq(
            pdpService.getSumTreeCounts(testSetId, 7),
            87,
            "Incorrect sum tree count"
        );
        assertEq(
            pdpService.getRootLeafCount(testSetId, 7),
            34,
            "Incorrect root leaf count"
        );
        Cids.Cid memory expectedCid = Cids.Cid(
            abi.encodePacked("test", uint256(3))
        );
        Cids.Cid memory actualCid = pdpService.getRootCid(testSetId, 3);
        assertEq(actualCid.data, expectedCid.data, "Incorrect root CID");
    }

    function setUpTestingArray()
        public
        returns (
            uint256[] memory counts,
            uint256[] memory expectedSumTreeCounts
        )
    {
        ...
        for (uint256 i = 0; i < counts.length; i++) {
            Cids.Cid memory testCid = Cids.Cid(abi.encodePacked("test", i));
            PDPService.RootData[]
                memory rootDataArray = new PDPService.RootData[](1);
            rootDataArray[0] = PDPService.RootData(
                testCid,
                counts[i] * pdpService.LEAF_SIZE()
            );
            pdpService.addRoots(testSetId, rootDataArray);
            ...
            assertEq(
                pdpService.getRootCid(testSetId, i).data,
                testCid.data,
                "Root not added correctly"
            );
        }
        ...
        for (uint256 i = 0; i < rootIdsToRemove.length; i++) {
            ...
            assertEq(
                pdpService.getRootCid(testSetId, rootIdsToRemove[i]).data,
                zeroBytes
            );
            assertEq(
                pdpService.getRootLeafCount(testSetId, rootIdsToRemove[i]),
                0,
                "Root size should be 0"
            );
        }
    }
    ...
    function testSumTree() public {
        ...
        for (uint256 i = 0; i < counts.length; i++) {
            assertEq(
                pdpService.getSumTreeCounts(testSetId, i),
                expectedSumTreeCounts[i],
                "Incorrect sum tree size"
            );
        }
    ...
    function assertFindRootAndOffset(
        uint256 setId,
        uint256 searchIndex,
        uint256 expectRootId,
        uint256 expectOffset
    ) internal view {
        uint256[] memory searchIndices = new uint256[](1);
        searchIndices[0] = searchIndex;
        PDPService.RootIdAndOffset[] memory result = pdpService.findRootIds(
            setId,
            searchIndices
        );
        if (result[0].rootId != expectRootId) {
            revert TestingFindError(
                expectRootId,
                result[0].rootId,
                "unexpected root"
            );
    ...
    function assertFindRootsAndOffsets(
        uint256 setId,
        uint256[] memory searchIndices,
        uint256[] memory expectRootIds,
...
contract RecordKeeperHelper is Test {
    ...
    function assertProofSetRecords(uint256 setId) public view {
        PDPRecordKeeper.EventRecord[] memory records = PDPRecordKeeper(
            recordKeeper
        ).listEvents(setId);
        assertEq(
            expectedRecords[setId].length,
            records.length,
            "Incorrect number of records"
        );
        for (uint256 i = 0; i < records.length; i++) {
            assertEq(
                uint(records[i].operationType),
                uint(expectedRecords[setId][i]),
                "Incorrect operation type"
            );
            assertRecordDataFormat(
                records[i].operationType,
                records[i].extraData
            );
        }
    }
    ...
    function assertRecordDataFormat(
        PDPListener.OperationType operationType,
        ...
        if (operationType == PDPListener.OperationType.CREATE) {
            ...
            (uint256 totalDelta, uint256[] memory rootIds) = abi.decode(
                extraData,
                (uint256, uint256[])
            );
            ...
            operationType == PDPListener.OperationType.PROVE_POSSESSION
        ) {
